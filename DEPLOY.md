...
（アカウント・実行履歴・出力した一覧表）だけをここに置く。全部あわせて数百KB。
...
3. 編集者に伝えること
   - URLとID・パスワード
   - **完成版（カット済み）を1本アップロードする**こと。
     使うのは音声だけなので、編集ソフトから音声だけ書き出せるならそのほうが速い
   - 出てきた一覧表（xlsx）はDriveに手で格納すること。
     Drive上で右クリック →「アプリで開く」→ Googleスプレッドシート で開ける
